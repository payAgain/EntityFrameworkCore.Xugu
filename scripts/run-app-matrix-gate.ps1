<#
.SYNOPSIS
  Phase 13.102 â€?Application capability matrix gate (native + compat).
  Requires a live XuguDB; Skip is treated as failure via XUGU_REQUIRE_DATABASE=true.

.EXAMPLE
  scripts/run-app-matrix-gate.ps1
  scripts/run-app-matrix-gate.ps1 -Configuration Release -DialectMode native
#>
param(
