...
仓库契约交叉引用：`harness/contracts/ado-driver-contract.md`（DateTimeOffset 读/写/相等过滤行）。ADO 直绑**写入时完整字符串**可为 1，但 `TIMESTAMP '…'` 字面量与 `GetString` 截断往返常为 0——Provider **无可靠旁路**，GearsOfWar 相关过滤测试保持 `Driver limitation (DRV-06)` Skip。
