...
.EXAMPLE
  harness/scripts/publish-nuget.ps1 -Pack -Configuration Release
...
$ciCmd = "harness/scripts/ci-build.ps1 -Configuration $Configuration -Pack"
if ($NativeDllPath) {
...
if ($Push) {
    ...
    Write-Host "  (push skipped — set GITLAB_NUGET_FEED_URL and use -Push to enable)"
}
Write-Host ""
