...
- `RelationalMaxIdentifierLengthConvention(127)`（与文档一致）
- `SqlGenerationHelper.DelimitIdentifier()` 使用反引号 `` ` ``
...
| `byte[]` | 无长度 → `BLOB`；有 `MaxLength`/size（≤64KB）→ `BINARY`；**均不**生成 `BLOB(n)`/`BINARY(n)`（文档 DDL 无 size 后缀） | `datatype/binary.md`、`datatype/large-object.md` |
...
| `byte[].Contains(b)` | `LOCATE(LPAD(HEX(b),2,'0'), HEX(arr)) > 0`（BLOB 无直接 LOCATE） | 旧：`LOCATE(b, arr)` | QueryTranslators | done |
...
| byte[] Contains | `LOCATE(LPAD(HEX(b),2,'0'), HEX(src)) > 0` | 同左（BLOB 旁路） | XuguByteArrayMethodTranslator |
| byte[] First / indexer | `CONV(SUBSTRING(HEX(src),…),16,10)` | MySQL: `ASCII(SUBSTRING…)` | XuguByteArrayMethodTranslator / SqlTranslatingExpressionVisitor |
| string `+`（ExecuteUpdate） | `CONCAT(a, b)`（禁止 `+`，E17003） | 同左 | XuguQuerySqlGenerator.VisitSqlBinary |
...
| TimeSpan/TimeOnly 物化 | `GetString` + `CustomizeDataReaderExpression`（无 ValueConverter 叠层） | converter-only shaper | XuguTimeSpan/TimeOnlyTypeMapping |
| DateTimeOffset.DayOfYear | `DAYOFYEAR(DATE(timestamptz))`（直接 DAYOFYEAR(timestamptz)→E10049/E17007） | — | XuguDateTimeMemberTranslator |
| DML CROSS JOIN | 实库 E19132；SELECT CROSS JOIN 有文档 | MySQL multi-table UPDATE | Northwind Bulk 负向 |
