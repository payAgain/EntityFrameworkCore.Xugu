...
**与 MySQL/Pomelo 差异**：Xugu 单表 DELETE 使用 `DELETE FROM table`（非 MySQL `DELETE alias FROM table`）；多表使用双 `FROM` 子句（`DELETE FROM t1 FROM t2`），非 MySQL 逗号 JOIN 列表。

**Phase 7 范围外**：`RETURNING`、TPC/TPT 继承批量更新、Owned JSON 列、实体拆分。
