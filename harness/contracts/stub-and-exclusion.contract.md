...
| 测试 | 原因 | disposition |
|------|------|-------------|
| `OptimisticConcurrencyTests.Stale_*` | E10049 ROW_COUNT | **signed-off blocked**（12.509/PLAT-01） | W5 |
| `WithConstructorsTests` graph insert | constructor insert → `DbUpdateException`（可执行断言，非 Skip） | **excluded-with-evidence**（12.312） |
| `ComplexTypesTrackingTests` optional null | EF #31376；模型 `IsNullable=false` + SaveChanges 拒绝 null | **excluded-with-evidence**（12.313） |
| `LazyLoadTests` proxy | 无 proxy 宿主；断言未加载直至 Explicit Load（OOS-08） | **excluded-with-evidence**（12.410） |
| `SeedingTests.EnsureCreated_*` | 共享 SYSTEM 上 `EnsureCreated` 返回 false、不应用 HasData | **excluded-with-evidence**（12.410） |
| Sequence `RESTART WITH` / HiLo | Alter 文档无 RESTART；HiLo 未做 | **NotSupported / defer**（2026-07-21） |
| Spec Functional W2+ | Northwind Spec 全量替换、Interception 全家桶等 | **defer**（W1 已落地，见 `2026-07-21-spec-matrix-alignment-design.md`） |
