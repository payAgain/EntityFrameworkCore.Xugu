...
### 7.3 测试 Skip（6 方法 — 待 W11/W12/W13 处置）

| 测试 | 原因 | 目标 disposition |
|------|------|------------------|
| `OptimisticConcurrencyTests.Stale_*` | E10049 ROW_COUNT | blocked → W13 或 exclusion |
| `WithConstructorsTests` ×2 | constructor insert | W12 implement 或 exclusion |
| `ComplexTypesTrackingTests` ×1 | EF #31376 | W12 或 exclusion |
| `LazyLoadTests` ×1 | 无 proxy 宿主 | W11 exclusion |

### 7.4 永久 skip 模块（W14 — 不计入 Adjusted 分母）

| 模块 | Pomelo 估算测试 | Path B |
|------|----------------|--------|
| NTS / Spatial | ~30 | doc exclusion |
| FULLTEXT / Match | ~15 | REGEXP_LIKE 或 exclusion |
| Collation / HasCharSet | ~10 | doc exclusion |
| CONVERT_TZ | ~5 | doc exclusion |
| Scaffolding Baselines 全量 | ~20 | 最小集或 exclusion |
