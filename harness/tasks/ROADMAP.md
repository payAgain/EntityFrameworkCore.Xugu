...
## 当前 Phase: **11**（`done` — W1–W9 complete；**2.1.0**）

**版本**：**`2.1.0`**  
**测试**：**898** 列测（compat 矩阵；~85% Pomelo 覆盖）  
**Wave 指针**：W1–W9 **done**；偏差修复轨 W3–5 ✅；发布轨 W6–W9 ✅
...
| **11** | **Xugu 原生方言 / 2.1.0** | **`done`** | **2.1.0** | JSON Provider、RETURNING、双 CI、NuGet 门禁 |
...
| **W3** | **11.501–11.505** | RETURNING 实库调研、capability 策略、`XuguUpdateSqlGenerator`、identity 测试 | W2 done | 11.M5 ✅；native identity 实库 PASS | **done** |
| **W4** | **11.601–11.604** | 默认 compat off、标识符审计、compat/native 双 CI 矩阵 | W3 done | 11.M6 ✅；898 compat + native 核心 0 FAIL | **done** |
| **W5** | **11.701–11.705** | contract/docs/Release Gate；compat 弃用路径（可选） | W4 done | 11.M7 ✅ | **done** |
| **W6** | **11.301–11.303** | NuGet 打包门禁、LIMITATIONS 冻结、版本与 CHANGELOG | **W5 done** | `publish-nuget.ps1 -Pack` PASS | **done** |
| **W7** | **11.208, 11.304, 11.305** | 连接串校验器、集成样本骨架、GETTING-STARTED 2.1.0 | W6 done | 集成样本可跑通 | **done** |
| **W8** | **11.401–11.403** | FunctionalTests 余量、Specification Phase 2 | W6 done | 898 列测 0 FAIL | **done** |
