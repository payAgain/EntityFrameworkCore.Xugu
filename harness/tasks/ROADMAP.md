...
## 当前 Phase: **11**（`in_progress` — Wave 1 done；Wave 2 JSON Provider 进行中；Wave 3–5 偏差修复轨已规划）

**版本**：**`2.0.0`** 稳定基线 → Phase 11 目标 **`2.1.0`**  
**测试**：**861** 列测（~82% Pomelo 覆盖）  
**源码**：Xugu **133** .cs vs Pomelo **194** .cs（~69%）  
**Wave 指针**：Phase 10 closure ✅ → **W1 done** → **W2 进行中**（11.109）→ **W3–5 偏差修复轨 todo**（见 `NATIVE-DIALECT-ROADMAP.md`）→ **W6+ 发布轨**
...
| **11 偏差修复轨** | **`harness/tasks/phase-11-xugu-native-release/NATIVE-DIALECT-ROADMAP.md`**（W3–5：RETURNING / compat flip / 双矩阵） |
...
> **偏差修复轨（W3–5）**：`harness/tasks/phase-11-xugu-native-release/NATIVE-DIALECT-ROADMAP.md`  
...
| **11.M5** | **Native Identity** | 原生模式 INSERT identity 回读 0 FAIL；`INSERT … RETURNING` 主路径 | **W3 偏差修复** |
| **11.M6** | **Compat Optional** | 默认 compat off；dual CI（compat 861 + native 核心）green | **W4 偏差修复** |
| **11.M7** | **方言契约闭环** | contract / XUGU-VS-MYSQL / Release Gate 反映 native-first | **W5 偏差修复** |
...
| **W2** | **11.109**（a→d） | `XuguJsonTypeMapping`、JSON 路径/函数 LINQ、Fluent API、实库测试子集 | W1 done ✅ | JSON 实库 PASS；`verify-module.ps1` Storage/Query PASS | **in_progress** |
| **W3** | **11.501–11.505** | RETURNING 实库调研、capability 策略、`XuguUpdateSqlGenerator`、identity 测试 | W2 done | 11.M5 ✅；native identity 实库 PASS | **todo** |
| **W4** | **11.601–11.604** | 默认 compat off、标识符审计、compat/native 双 CI 矩阵 | W3 done | 11.M6 ✅；861 compat + native 核心 0 FAIL | **todo** |
| **W5** | **11.701–11.705** | contract/docs/Release Gate；compat 弃用路径（可选） | W4 done | 11.M7 ✅ | **todo** |
| **W6** | **11.301–11.303** | NuGet 打包门禁、LIMITATIONS 冻结、版本与 CHANGELOG | **W5 done**（建议） | `test-nuget-pack.ps1` PASS；dual matrix 0 FAIL | **todo** |
| **W7** | **11.208, 11.304, 11.305** | 连接串校验器、集成样本骨架、GETTING-STARTED 2.1.0 | W6 done | 集成样本 README 可跑通（有实库时） | **todo** |
| **W8** | **11.401–11.403** | FunctionalTests 余量、Specification Phase 2、Monster Fixup 扩展 | W6 done | 列测回归 0 FAIL（增量可选 +20~40） | **todo** |
| **W9** | **11.105, 11.205, 11.207, 11.107, 11.202–11.204** | ROW_COUNT、Linux RID、DateOnly SC、net8.0 TFM、FOR UPDATE 等 | 驱动/ROI 解锁 | 独立 minor/patch；**不合并进 2.1.0 门禁** | **optional** |
