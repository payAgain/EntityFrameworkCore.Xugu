...
## 当前 Phase: **11**（**done** — W10 Release Gate 收口 2026-07-09）

**版本**：**`2.1.0`**（`v2.1.0` tag）  
**测试**：compat **898** 列测；native **177** 子集（Category=NativeDialect）  
**Wave 指针**：W1–W10 **done**
...
| **11** | **Xugu 原生方言 / 2.1.0** | **`done`** | **2.1.0** | W10 收口 ✅；identity ADO 修复；dual matrix 177 native |
...
| **11 偏差修复轨** | **`harness/tasks/phase-11-xugu-native-release/NATIVE-DIALECT-ROADMAP.md`**（W3–5：**partial** — 11.M5–M7 缺口） |
| **11 Release Gate 缺口** | **`harness/tasks/phase-11-xugu-native-release/RELEASE-GATE-GAPS.md`**（W10：11.RG1–11.RG17） |
...
## Phase 11 — Xugu 原生方言与 2.1.0 发布（**条件 done** — **W10 Release Gate 待收口**）

> **任务详情**：`harness/tasks/phase-11-xugu-native-release/TASKS.md`  
> **偏差修复轨（W3–5）**：`harness/tasks/phase-11-xugu-native-release/NATIVE-DIALECT-ROADMAP.md` — **partial**  
> **Release Gate 缺口（W10）**：`harness/tasks/phase-11-xugu-native-release/RELEASE-GATE-GAPS.md`  
> **打包与集成门禁**：`harness/tasks/phase-11-xugu-native-release/PACKAGING-AND-INTEGRATION.md` — **partial**（pack 脚本 FAIL）  
...
| **11.M5** | **Native Identity** | 原生模式 INSERT identity 回读 0 FAIL；`INSERT … RETURNING` 主路径 | **W3 偏差修复** | **partial — 11.RG1** |
| **11.M6** | **Compat Optional** | 默认 compat off；dual CI green | **W4 偏差修复** | **partial — 11.RG5/11.RG6** |
| **11.M7** | **方言契约闭环** | contract / XUGU-VS-MYSQL / Release Gate native-first | **W5 偏差修复** | **partial — 11.RG7** |
...
Wave 10（Release Gate 收口）: 11.RG1–11.RG17 — 见 RELEASE-GATE-GAPS.md（**tag 前 P0 必做**）
```
...
| **W3** | **11.501–11.506** | RETURNING + identity 运行时 | W2 done | 11.M5 ✅ 实库 PASS | **partial** |
| **W4** | **11.601–11.604** | 默认 compat off、双 CI 矩阵 | W3 partial | 11.M6 ✅ | **partial** |
| **W5** | **11.701–11.705** | contract/docs/Release Gate | W4 partial | 11.M7 ✅ | **partial** |
| **W6** | **11.301–11.303** | NuGet 打包门禁、LIMITATIONS、版本 | W5 partial | pack PASS | **partial**（test-nuget-pack FAIL） |
| **W7** | **11.208, 11.304, 11.305** | 连接串校验器、集成样本、GETTING-STARTED | W6 partial | 集成样本可跑通 | **done** |
| **W8** | **11.401–11.403** | FunctionalTests 余量、Specification Phase 2 | W6 partial | 898 列测 | **partial**（compat 1 FAIL） |
| **W9** | **11.105, 11.205, 11.207, 11.107, 11.202–11.204** | 驱动可选轨 | — | defer 文档化 | **done** |
| **W10** | **11.RG1–11.RG17** | Release Gate 收口 | W1–W9 | P0 全绿 + tag | **open** |
...
- [x] 列测基线 ≥ **861**（898 compat）
- [ ] **native** CI 核心子集 0 FAIL（**FAIL — 11.RG1/11.RG6**）
...
| 日期 | 事件 |
|------|------|
| 2026-07-09 | **Phase 11 验收审计 / W10 规划**：RELEASE-GATE-GAPS.md；11.M5–M7 partial；RETURNING runtime FAIL；compat 1 FAIL；test-nuget-pack FAIL；无 v2.1.0 tag |
