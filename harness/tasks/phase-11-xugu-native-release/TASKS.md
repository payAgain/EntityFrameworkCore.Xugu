...
> **状态**：`in_progress`（Wave 1 done；Wave 2 待开工）  
...
| 11.M3 | 测试与打包门禁 | `PACKAGING-AND-INTEGRATION.md` 全流程 PASS；列测 ≥ **880** 或维持 861 且 0 FAIL |
...
| 11.109 | **JSON Provider 实现** | Storage + Query | 10.108 | todo | 见下表「11.109 子任务」 |
...
Wave 5（P2）    : 11.401–11.403 — 测试深化（可与 Wave 4 并行）
Wave 6（可选轨）: 11.105 / 11.205 / 11.207 / 11.107 — 仅驱动解锁后执行，不挡 2.1.0
```
...
| W3 | W2 done | `test-nuget-pack.ps1` PASS；全量 `dotnet test` 0 FAIL |
| W4 | W3 done | 集成样本 README 可跑通（有实库时） |
| W5 | W3 done | 列测回归 0 FAIL（增量可选） |
| W6 | 驱动发布 | 独立 minor/patch，不合并进 2.1.0 门禁 |
